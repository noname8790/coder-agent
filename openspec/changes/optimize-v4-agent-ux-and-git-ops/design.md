...
- Agent 消息使用轻量 Markdown 安全渲染，并支持复制消息原文。
- Agent 修改文件后展示 Diff 摘要，默认展示前 3 个文件，展开展示全部文件和截断 patch 片段。
...
### 6. Markdown 渲染必须轻量且安全降级

前端使用自研轻量 Markdown 渲染器渲染 Agent 消息，当前支持标题、列表、行内代码、代码块和状态行等 coding agent 常用格式。渲染器不执行原始 HTML，不把模型输出转换为可执行 DOM；遇到不支持的 Markdown 语法时按安全文本降级显示。复制按钮复制消息原始文本，不复制渲染后的 HTML。
...
- [Risk] Markdown 渲染引入 XSS 风险。→ 使用轻量渲染器，禁用 raw HTML，链接和表格等未支持语法按文本降级，代码块只渲染文本。
...
- Diff 展开后已采用“文件列表 + 增删统计 + 截断 patch 片段”的实现；后续若需要完整 patch 浏览，可基于工件路径按需读取。
