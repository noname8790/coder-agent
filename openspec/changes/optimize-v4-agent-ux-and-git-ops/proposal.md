...
- Agent 消息正文使用轻量 Markdown 安全渲染，并在每条用户/Agent 消息下方提供复制图标。
- Agent 任务改动文件后，在 Agent 消息下方展示 Diff 摘要：默认折叠显示前 3 个文件，展开后显示全部变更文件、增删行统计和截断 patch 片段。
...
- 依赖影响：
  - 前端使用轻量 Markdown 安全渲染器和安全降级策略，不引入 raw HTML 渲染能力。
