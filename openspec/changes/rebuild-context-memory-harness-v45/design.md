...
任务结束时系统还会生成一份不展示给用户的内部摘要。该摘要使用当前任务模型优先生成结构化 JSON，失败时回退规则摘要；内容包括用户目标、完成事项、涉及文件、验证结果、后续事项和风险。后续同一会话的旧消息压缩优先读取这些 `RUN_SUMMARY` 内部摘要进入 `<context>`，只有找不到内部摘要时才退回按历史消息做规则压缩。这样可以避免把长篇 Agent 回复原文截断后塞回 prompt。
...
文件读取和文件变更写入记忆时不保存文件全文或变更后源码原文。系统优先生成结构化内部摘要，保留 path、language、symbols、change_type、hash、verification 和 risk 等字段；模型摘要失败时使用规则摘要兜底。pgvector chunk 存储摘要文本而不是原始文件内容。
